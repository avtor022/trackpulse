...
- Мониторинг заездов с таймером и таблицей участников
- **Отображение регистрации кругов**: работоспособность транспондера, номер участника, ФИО, модель, масштаб, количество кругов, лучшее время круга
- Логи и отладка
...
│   │   ├── competition_participant_service.go  # Участники заезда + регистрация кругов
│   │   ├── lap_service.go    # Обработка RFID, подсчёт кругов, markTransponderWorked
...
**Назначение**: Обработка RFID-сканирований в реальном времени, подсчёт кругов, ведение результатов заезда, установка флага работоспособности транспондера.
...
- Методы: `Start()`, `Stop()`, `SetActiveCompetition()`, `ProcessScan()`, `QueueScan()`, `markTransponderWorked()`
- Структуры: `LapScan`, `ParticipantLapData`

#### Метод markTransponderWorked
При первом проезде участника через антенну автоматически вызывается метод `markTransponderWorked(participantID)`, который:
1. Получает участника из репозитория
2. Устанавливает флаг `TransponderWorked = true`
3. Сохраняет изменения в БД

Этот флаг отображается в таблице регистрации на панели мониторинга как "✓" (работает) или "✗" (не работал).
