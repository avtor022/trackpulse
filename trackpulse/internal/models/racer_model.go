...
func (rm *RacerModel) GetByID(db *sql.DB, id string) error {
	...
	return db.QueryRow(query, id).Scan(&rm.ID, &rm.RacerID, &rm.RcModelID, &rm.TransponderNumber, &rm.TransponderType, &rm.IsActive, &rm.CreatedAt, &rm.UpdatedAt)
}
...
func (rm *RacerModel) GetAll(db *sql.DB) ([]RacerModel, error) {
	...
	var models []RacerModel
	for rows.Next() {
		var model RacerModel
		err := rows.Scan(&model.ID, &model.RacerID, &model.RcModelID, &model.TransponderNumber, &model.TransponderType, &model.IsActive, &model.CreatedAt, &model.UpdatedAt)
		if err != nil {
			return nil, err
		}
