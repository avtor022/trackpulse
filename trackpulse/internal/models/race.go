...
func (r *Race) GetByID(db *sql.DB, id string) error {
	...
	return db.QueryRow(query, id).Scan(&r.ID, &r.RaceTitle, &r.RaceType, &r.ModelType, &r.ModelScale, &r.TrackName, &r.LapCountTarget, &r.TimeLimitMinutes, &r.TimeStart, &r.TimeFinish, &r.Status, &r.CreatedAt, &r.UpdatedAt)
}
...
func (r *Race) Update(db *sql.DB) error {
	...
	query := `UPDATE races SET race_title = ?, race_type = ?, model_type = ?, model_scale = ?, track_name = ?, lap_count_target = ?, time_limit_minutes = ?, time_start = ?, time_finish = ?, status = ?, updated_at = ? 
	          WHERE id = ?`
	
	_, err := db.Exec(query, r.RaceTitle, r.RaceType, r.ModelType, r.ModelScale, r.TrackName, r.LapCountTarget, r.TimeLimitMinutes, r.TimeStart, r.TimeFinish, r.Status, r.UpdatedAt, r.ID)
	return err
}
...
func (r *Race) GetAll(db *sql.DB) ([]Race, error) {
	...
	for rows.Next() {
		var race Race
		err := rows.Scan(&race.ID, &race.RaceTitle, &race.RaceType, &race.ModelType, &race.ModelScale, &race.TrackName, &race.LapCountTarget, &race.TimeLimitMinutes, &race.TimeStart, &race.TimeFinish, &race.Status, &race.CreatedAt, &race.UpdatedAt)
		if err != nil {
			return nil, err
		}
