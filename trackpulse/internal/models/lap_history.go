...
func (lh *LapHistory) Create(db *sql.DB) error {
	...
	_, err := db.Exec(query, lh.ID, lh.RaceParticipantID, lh.LapNumber, lh.LapTimeMs, 
	                  lh.StartTime, lh.EndTime, lh.IsValid, lh.InvalidationReason, now)
	if err != nil {
		return err
	}
...
func (lh *LapHistory) GetByID(db *sql.DB, id string) error {
	...
	err := db.QueryRow(query, id).Scan(
		&lh.ID, &lh.RaceParticipantID, &lh.LapNumber, &lh.LapTimeMs, &lh.StartTime, &lh.EndTime, 
		&lh.IsValid, &lh.InvalidationReason, &lh.CreatedAt,
	)
	return err
}
...
func (lh *LapHistory) Update(db *sql.DB) error {
	query := `UPDATE lap_history SET race_participant_id = ?, lap_number = ?, lap_time_ms = ?, start_time = ?, 
	                            end_time = ?, is_valid = ?, invalidation_reason = ? 
	          WHERE id = ?`
	
	_, err := db.Exec(query, lh.RaceParticipantID, lh.LapNumber, lh.LapTimeMs, lh.StartTime, 
	                  lh.EndTime, lh.IsValid, lh.InvalidationReason, lh.ID)
	if err != nil {
		return err
	}
...
func (lh *LapHistory) GetAll(db *sql.DB) ([]LapHistory, error) {
	query := `SELECT id, race_participant_id, lap_number, lap_time_ms, start_time, end_time, is_valid, invalidation_reason, created_at 
	          FROM lap_history ORDER BY start_time DESC`
