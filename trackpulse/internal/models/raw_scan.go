...
func (rs *RawScan) Create(db *sql.DB) error {
	...
	_, err := db.Exec(query, rs.ID, rs.Timestamp, rs.TagValue, rs.ReaderType, rs.COMPort, 
	                  rs.SignalStrength, rs.IsProcessed, rs.LinkedRacerModelID, now)
	if err != nil {
		return err
	}
...
func (rs *RawScan) GetByID(db *sql.DB, id string) error {
	...
	err := db.QueryRow(query, id).Scan(
		&rs.ID, &rs.Timestamp, &rs.TagValue, &rs.ReaderType, &rs.COMPort, &rs.SignalStrength, 
		&rs.IsProcessed, &rs.LinkedRacerModelID, &rs.CreatedAt,
	)
	return err
}
...
func (rs *RawScan) Update(db *sql.DB) error {
	query := `UPDATE raw_scans SET timestamp = ?, tag_value = ?, reader_type = ?, com_port = ?, 
	                            signal_strength = ?, is_processed = ?, linked_racer_model_id = ? 
	          WHERE id = ?`
	
	_, err := db.Exec(query, rs.Timestamp, rs.TagValue, rs.ReaderType, rs.COMPort, 
	                  rs.SignalStrength, rs.IsProcessed, rs.LinkedRacerModelID, rs.ID)
	if err != nil {
		return err
	}
