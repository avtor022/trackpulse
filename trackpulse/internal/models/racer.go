...
func (r *Racer) GetByID(db *sql.DB, id string) error {
	...
	return db.QueryRow(query, id).Scan(&r.ID, &r.RacerNumber, &r.FullName, &r.Birthday, &r.Country, &r.City, &r.Rating, &r.CreatedAt, &r.UpdatedAt)
}
...
func (r *Racer) GetAll(db *sql.DB) ([]Racer, error) {
	...
	for rows.Next() {
		var racer Racer
		err := rows.Scan(&racer.ID, &racer.RacerNumber, &racer.FullName, &racer.Birthday, &racer.Country, &racer.City, &racer.Rating, &racer.CreatedAt, &racer.UpdatedAt)
		if err != nil {
			return nil, err
		}
